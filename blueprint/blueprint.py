...
from blueprint.prompt import generate_task_prompt
from pydantic import BaseModel
from typing import Optional
from blueprint.utils import generate_crypto_string
from blueprint.auth import verify_access_token, refresh_access_token
...
def gemini_response(prompt: str):
  ...
  return client.models.generate_content( model="gemini-2.0-flash", contents=f"{prompt}")
...
def verify_response_values(sanitized_response) -> Optional[SanitizedResponse]:
  ...
  """
  if sanitized_response.get("info"): return None
  ...
  except ValueError as e: raise ValueError(f"Gemini returned invalid ISO 8601 date format: {e}")
...
def is_slot_free(service, start_time_iso, end_time_iso, calendar_id='primary') -> bool:
  ...
  log.info(f"Checking availability from {start_time_iso} to {end_time_iso}...")
  try:
    ...
    try: results = service.freebusy().query(body=freebusy_query).execute()
    except Exception as e:
      log.error(f"Error executing free busy query {e}")
    ...
    if not calendar_busy_times:
      log.info("Slot is free.")
      return True
    else:
      log.info(f"Slot is busy. Conflicts: {calendar_busy_times}")
      return False
  except HttpError as e:
    log.error(f"An API error occurred during free/busy check: {e}")
    return False
  except Exception as e:
    log.error(f"An unexpected error occurred during free/busy check: {e}")
    return False
...
def find_next_free_slot( service, original_start_dt, original_end_dt, calendar_id='primary'):
  ...
  while current_start_dt < search_limit_dt:
    ...
    if is_slot_free(service, start_iso, end_iso, calendar_id):
      log.info(f"Found next available slot: {start_iso} to {end_iso}")
      return start_iso, end_iso

    current_start_dt += relativedelta(minutes=FIND_NEXT_SLOT_INCREMENT_MINUTES)

  log.info(f"Could not find a free slot within the next {FIND_NEXT_SLOT_MAX_HOURS} hours.")
  return None, None
...
def create_calendar_service(redis_client: Redis, user_id: str):
  ...
  if not is_verified:
    ...
    if not new_access_token or not is_refreshed:
      log.error(f"Failed to refresh access token for user {user_id}")
      return None
    else:
      try:
        ...
        log.error(f"Failed to update access token in Redis: {e}")
        return None
  ...
  try: service = build('calendar', 'v3', credentials=credentials)
  except Exception as e:
    log.error(f"Failed to build Google Calendar service client: {e}")
    return None
  return service
...
def add_calendar_event(
  ...
  try:
    ...
    log.error(f"ERROR: Could not parse date/time from Gemini: {e}")
    return False
  ...
  if not is_slot_free(service, start_iso, end_iso, calendar_id):
    ...
    if not new_start_iso:
      log.error("Failed to find an alternative slot. Event not added.")
      return False
  ...
  try:
    ...
    log.error(f"A API error occurred while adding event: {e}")

    if hasattr(e, 'content'):
      try:
        error_details = json.loads(e.content)
        log.error(f"Error details: {json.dumps(error_details, indent=2)}")
      except json.JSONDecodeError: log.error(f"Error content: {e.content}")
    return False
  except Exception as e:
