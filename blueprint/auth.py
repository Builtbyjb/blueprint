...
def verify_access_token(access_token: str) -> bool:
  ...
  try:
    # Verify access token
    response = requests.get( TOKEN_INFO_URL, params={'access_token': access_token}, timeout=10)
...
def refresh_access_token(refresh_token: str) -> tuple[Optional[str], bool]:
  ...
  try:
    ...
    log.error(f"Token Refresh Error: Network error ({e}).")
    return None, False
...
def isAuth(redis_client: Redis, user_id: str) -> bool:
  ...
  if not is_verified_token:
    ...
    if not is_refreshed or not new_access_token: return False
    try:
